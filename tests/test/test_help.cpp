...
int main() {
        std::cout<<std::endl<<"------ TEST COMMAND:     ./a.out init  &&  ./a.out help  &&  ./.a.out help stage       ------"<<std::endl;

 Project* project = new Project();
 void* output;

 char* argv1[2] = {"./a.out","init"};
 char* argv3[3] = {"./a.out","help","status"};

 Command * command1 = new initCommand(1,argv1+1);
 Command * command3 = new helpCommand(1,argv3+1);
 Command * command4 = new helpCommand(2,argv3+1);
